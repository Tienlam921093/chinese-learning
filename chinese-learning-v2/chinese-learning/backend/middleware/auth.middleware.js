...
async function authenticate(req, res, next) {
  ...
  try {
    ...
    if (!dbUser || !dbUser.is_active)
      return res.status(401).json({ message: 'Tài khoản không tồn tại hoặc đã bị vô hiệu hoá.' });

    if (dbUser.token_version !== decoded.token_version)
      return res.status(401).json({
        message:     'Phiên đăng nhập đã hết hạn do mật khẩu thay đổi. Vui lòng đăng nhập lại.',
