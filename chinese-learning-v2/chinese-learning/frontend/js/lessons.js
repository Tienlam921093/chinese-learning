...
function showUpgradeToast(hskLevel) {
  ...
  upgradeToastTimer = setTimeout(() => { if (toast.parentNode) toast.remove(); }, 4000);
}

const DEMO_LESSONS = [
  // HSK 1
  { id:1,  hsk:1, title:'Chào Hỏi Cơ Bản',    desc:'你好 · 谢谢 · 对不起 · 再见',      words:12, duration:15, completed:false, emoji:'👋' },
  { id:2,  hsk:1, title:'Số Đếm 1-10',          desc:'一 · 二 · 三 · 四 · 五 · 十',      words:10, duration:10, completed:false, emoji:'🔢' },
  { id:3,  hsk:1, title:'Màu Sắc',              desc:'红色 · 蓝色 · 绿色 · 黄色',        words:8,  duration:12, completed:false, emoji:'🎨' },
  { id:4,  hsk:1, title:'Gia Đình',             desc:'爸爸 · 妈妈 · 哥哥 · 姐姐',        words:14, duration:18, completed:false, emoji:'👨‍👩‍👧‍👦' },
  { id:5,  hsk:1, title:'Thức Ăn & Đồ Uống',   desc:'饭 · 水 · 茶 · 咖啡 · 面包',       words:16, duration:20, completed:false, emoji:'🍜' },
  { id:6,  hsk:1, title:'Thời Gian',            desc:'今天 · 明天 · 昨天 · 几点',         words:12, duration:15, completed:false, emoji:'🕐' },
  // HSK 2
  { id:7,  hsk:2, title:'Mua Sắm',              desc:'多少钱 · 便宜 · 贵 · 买 · 卖',      words:18, duration:22, completed:false, emoji:'🛍️' },
  { id:8,  hsk:2, title:'Giao Thông',           desc:'公交车 · 地铁 · 出租车 · 骑车',     words:15, duration:18, completed:false, emoji:'🚌' },
  { id:9,  hsk:2, title:'Sức Khỏe',             desc:'医院 · 医生 · 头疼 · 发烧',         words:20, duration:25, completed:false, emoji:'🏥' },
  // HSK 3
  { id:10, hsk:3, title:'Thời Tiết',            desc:'天气 · 下雨 · 晴天 · 刮风 · 温度', words:22, duration:28, completed:false, emoji:'⛅' },
  { id:11, hsk:3, title:'Du Lịch',              desc:'旅游 · 护照 · 酒店 · 飞机 · 签证', words:25, duration:30, completed:false, emoji:'✈️' },
  // HSK 4
  { id:12, hsk:4, title:'Công Việc',            desc:'工作 · 公司 · 上班 · 老板 · 薪水', words:30, duration:35, completed:false, emoji:'💼' },
  { id:13, hsk:4, title:'Cảm Xúc & Tính Cách', desc:'高兴 · 生气 · 难过 · 紧张 · 骄傲', words:28, duration:32, completed:false, emoji:'😊' },
  // HSK 5
  { id:14, hsk:5, title:'Kinh Tế & Tài Chính',  desc:'经济 · 股票 · 投资 · 银行 · 利率', words:40, duration:45, completed:false, emoji:'📈' },
  { id:15, hsk:5, title:'Môi Trường',           desc:'环境 · 污染 · 能源 · 保护 · 气候', words:38, duration:42, completed:false, emoji:'🌍' },
  // HSK 6
  { id:16, hsk:6, title:'Triết Học & Văn Hóa', desc:'哲学 · 文化 · 传统 · 思想 · 精神', words:55, duration:60, completed:false, emoji:'🏮' },
  { id:17, hsk:6, title:'Chính Trị & Xã Hội',  desc:'政治 · 社会 · 制度 · 民主 · 改革', words:60, duration:65, completed:false, emoji:'⚖️' },
];
...
function init() {
  ...
  const user = JSON.parse((localStorage.getItem('hanyuUser') || sessionStorage.getItem('hanyuUser'))     || '{}');
  ...
  DEMO_LESSONS.forEach(l => { l.completed = _done.has(l.id); });

  // Render — đọc ?level= từ URL nếu có
  const _urlLevel = parseInt(new URLSearchParams(location.search).get('level')) || 0;
  if (_urlLevel >= 1 && _urlLevel <= 6) {
    const _tab = document.querySelector(`#filterTabs .ftab[onclick*="filterLevel(${_urlLevel},"]`);
    if (_tab) { _tab.click(); }
    else { renderLessons(DEMO_LESSONS.filter(l => l.hsk === _urlLevel)); }
  } else {
  ...
  authFetch(`${API_BASE}/lessons`)
    ...
    .then(data => {
      ...
      normalized.forEach(api => {
        ...
        if (demo) { demo.dbId = api.id; demo.desc = api.desc || demo.desc; }
      });
      ...
      DEMO_LESSONS.forEach(l => { l.completed = _doneApi.has(l.id) || _doneApi.has(l.dbId); });
      // Re-render tab đang active
      const _activeTab = document.querySelector('#filterTabs .ftab.active');
      const _activeMatch = _activeTab ? (_activeTab.getAttribute('onclick') || '').match(/filterLevel\((\d+)/) : null;
      const _activeLevel = _activeMatch ? parseInt(_activeMatch[1]) : 0;
      const _list = _activeLevel > 0 ? DEMO_LESSONS.filter(l => l.hsk === _activeLevel) : [...DEMO_LESSONS];
      renderLessons(_list);
    })
    .catch(() => { /* giữ demo data, không làm gì */ });
}
...
function filterLevel(level, btn) {
  ...
  // Lọc data
  const list = level === 0 ? DEMO_LESSONS : DEMO_LESSONS.filter(l => l.hsk === level);
  renderLessons(list);
}
...
function openLesson(id) {
  ...
  document.getElementById('mVocabLink').href = `vocabulary.html?lesson=${id}&mode=list`;

  const colors = {1:'#16a34a',2:'#2563eb',3:'#d97706',4:'#7c3aed',5:'#dc2626',6:'#475569'};
  document.getElementById('modalHeader').style.background = colors[l.hsk] || '#0f172a';
  ...
  document.getElementById('mNextBtn').disabled = idx === DEMO_LESSONS.length - 1;
